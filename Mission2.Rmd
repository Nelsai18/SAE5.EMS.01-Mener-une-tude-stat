...
# 2. ÉVOLUTION TEMPORELLE DES POINTS DE RECHARGE 2015-2020

```{r}

# 2.1 Évolution cumulative mensuelle 
...
# Alternative si vous voulez compter les stations uniques
# Vérifier d'abord quelle colonne identifier les stations
...
cat("Évolution cumulative calculée avec succès\n")
```
...
```{r}
# 3.1 Répartition finale en 2020
...
carte_data <- bornes_clean %>%
  filter(annee == 2020) %>%
  sample_n(min(3000, nrow(.))) %>%
  mutate(
    popup_info = paste(
      "<b>", nom_amenageur, "</b><br>",
      "PDC:", nbre_pdc, "<br>",
      "Puissance:", puissance_nominale, "kW<br>",
      "Type:", categorie_puissance
    )
  )
...
pal <- colorFactor(
  palette = "YlOrRd", 
  domain = carte_data$categorie_puissance
)
...
leaflet(carte_data) %>%
  ...
  addCircleMarkers(
    ...
    radius = ~log(nbre_pdc + 1) * 3,
    color = ~pal(categorie_puissance),
    stroke = FALSE,
    fillOpacity = 0.8,
    popup = ~popup_info,
